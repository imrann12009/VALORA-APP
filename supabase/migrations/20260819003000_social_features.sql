...
-- Stories
create policy "active stories are readable" on stories for select to authenticated
using (expires_at > now() and (privacy_check_placeholder is null));

-- Replace the placeholder policy above with explicit access rules.
...
-- Story views
create policy "story views readable by story owner" on story_views for select to authenticated
...
with check (exists (select 1 from conversation_members cm where cm.conversation_id = conversation_id and cm.user_id = (select id from profiles where auth_user_id = auth.uid())) or user_id = (select id from profiles where auth_user_id = auth.uid()));
...
using (sender_id = (select id from profiles where auth_user_id = auth.uid()))
with check (sender_id = (select id from profiles where auth_user_id = auth.uid()));
...
with check (bucket_id = 'post-media' and (storage.foldername(name))[1] = (select auth_user_id::text from profiles where id = (select id from profiles where auth_user_id = auth.uid())));
